  #include<iostream>//Leetcode    328                Odd Even Linked List
#include<vector>
using namespace std;
...
int main(){
  ...
  ListNode* hi = new ListNode(-1);//pointer for those 2
        ListNode* tl = low;
        ListNode* th = hi;

        ListNode* temp = head;
        int n = 0;//here we need to partition the list according to index, odd index to left and even index to right
        while(temp != NULL){
            ++n;//for indexing
            if(n%2 != 0){//left values
                tl->next = temp;//connect tl's next to temp
                temp = temp->next;//move temp
                tl = tl->next;//move tl
            }
            else{//m%2 == 0   // //right values
