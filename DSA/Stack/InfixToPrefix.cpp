#include<iostream>//Infix to prefix expression
#include<stack>
#include<string>
using namespace std;
...
int main(){
    ...
    for(int i=0;i<s.length();i++){//traverse the string
        if(isdigit(s[i])){//if it is a digit then push it
    ...
    cout<<"prefix expression :"<<val.top();  //stack will be left our with expression    
}