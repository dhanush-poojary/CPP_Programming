#include<iostream>//weighted or unweighted  graph implmentation using adjecency list
#include<vector>
#include<list>
using namespace std;

vector<list<pair<int,int>>> graph;//create a adgecency list of pair to store node and weight
int v,e;
void add_edge(int so_idx,int de_idx,int wt){
     graph[so_idx].push_back({de_idx,wt});//insert destination node in the list of graph[source node,weight]
}
